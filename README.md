...
On the monitor PC itself, opening either `https://192.168.1.129:8766/` or the published Pages URL above connects to the local feed automatically without a bearer link. The Pages URL uses the local probe only on the PC; Android and other machines must use the private one-tap link.
...
- **Published Pages shell on the PC shows Token needed:** reload it on the monitor PC. It first probes `https://192.168.1.129:8766/` without a token; if this PC has not accepted the local HTTPS certificate yet, open that local address once and accept the browser's one-time certificate warning, then reload the Pages URL.
